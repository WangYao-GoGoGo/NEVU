...
"""Inference utilities for the Controlled Grouping Test.

The CGT path shares most generation and parsing logic with the main inference
module, but constructs controlled grouping payloads for heuristic-baseline
comparisons.
"""
...
def run_inference(
    ...
    if group.upper() == "G1":
        ...
        for filename, sub_test_data in enumerate(sub_test_data_files):
            ...
            for iid in sub_test_data_keys:
                ...
                if not e:
                    continue
                payload = data_utils.build_input_payload_cgt(e, iid, label_space, hv_label, hv_label_desc)
                actors = e["actors"]
    ...
    for iid in tqdm(list(gold.keys()), desc=f"Infer {group}:{model_name}"):
        ...
        gold_detail = gold[iid]
        random_subevent_ids = gold_detail["random_subevent_ids"]
        payload = data_utils.build_input_payload_cgt(e, iid, label_space, hv_label, hv_label_desc, random_subevent_ids)


        payload["random_subevent_ids"] = random_subevent_ids
        # ---------- Local (G2/G3) ----------
