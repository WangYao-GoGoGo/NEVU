...
def run_inference(
    ...
    if group.upper() == "G1":
        ...
        for filename, sub_test_data in enumerate(sub_test_data_files):
            ...
            for iid in sub_test_data_keys:
                ...
                if not e:
                    continue
                payload = data_utils.build_input_payload_pseudo(e, iid, label_space, hv_label, hv_label_desc)
                actors = e["actors"]
    ...
    for iid in tqdm(list(gold.keys()), desc=f"Infer {group}:{model_name}"):
        ...
        payload = data_utils.build_input_payload_pseudo(e, iid, label_space, hv_label, hv_label_desc, random_subevent_ids)
